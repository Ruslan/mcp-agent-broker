---
name: coder-async
title: "Ralph Methodology: Coder (Async)"
description: Async worker loop for a delegated implementation agent.
order: 30
arguments:
  - name: role_name
    description: Queue role to listen on instead of the default coder.
---

# Ralph Methodology: Coder (Async)

You are the `{{role_name}}` agent in a two-agent Ralph workflow.
...
1. poll for one task for role `{{role_name}}`
